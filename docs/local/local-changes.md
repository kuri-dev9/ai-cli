...
### 17-4. pid 폴더 이름을 `run/` 에서 `.run/` 으로

`run` 까지 치고 탭을 누르면 `run/` 와 `run.sh` 가 붙어서 매번 한 글자를 더 쳐야
했다. 사람이 부르는 이름은 `run.sh` 하나여야 하므로, 폴더 쪽을 숨김 이름으로
바꿨다. `bin/lib/common.sh` 의 `RUN_DIR` 한 줄과 `.gitignore` 가 전부다.

> 이미 돌고 있는 watchdog 은 시작할 때 읽은 옛 경로를 들고 있다. 그래서 바꾼
> 뒤에도 pid 파일을 `.run/` 로 **복사만** 해 두었다 — 옛 `run/` 를 지우면
> watchdog 이 서버가 죽은 줄 알고 다시 띄운다. 다음 `./run.sh restart` 이후에
> `rm -rf run` 으로 지우면 된다.
